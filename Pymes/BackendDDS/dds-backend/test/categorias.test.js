...
describe("GET /api/categorias/:id (no existe)", function () {
    it("debería devolver 404 si la categoría no existe", async function () {
        const res = await request(app).get("/api/categorias/999999");
        expect(res.statusCode).toEqual(404);
        expect(res.body).toEqual({ message: 'categoria no encontrado' });
    });
});
